...
def main(args, accelerator):
    ...
    tokenizer = RobertaTokenizer.from_pretrained("roberta-large", use_fast=True, cache_dir=CACHE_DIR)
    model = RobertaForMaskedLM.from_pretrained("roberta-large", torch_dtype=torch.float16, cache_dir=CACHE_DIR)
    accelerator.print('miniLM', file=sys.stderr)
    mpnet = SentenceTransformer("all-MiniLM-L6-v2", cache_folder = CACHE_DIR)
    

    
    datadir = f'./data/{DATASET_NAME}_train.json'
    ...
    eps, opt_alpha = privacy_analysis.get_privacy_spent(
            orders=[1.0 + 0.1 * t for t in range(1, 1000)], rdp=rdp, delta=DELTA
        )
    ...
    config = {
        ...
        "H": scale * 4.0 * H_MULTIPLIER, # formerly 4.0
    ...
    load_list = [x for x in load_list if len(x.split(" ")) > 10]
    init_pop = load_list
    ...
    schedule = [MASK for i in range(config["T"])]
    accelerator.print(output_dir, file=sys.stderr)
